...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({ error: 'API-Key darf nicht leer sein' }, { status: 401 })
    }
    ...
    const { data: validSensors, error: sensorError } = await supabase
      .from('sensors')
      .select('id')
      .in('id', sensorIds)
    ...
    if (sensorError) {
      return NextResponse.json({ error: 'Fehler beim Validieren der Sensoren: ' + sensorError.message }, { status: 500 })
    }
    ...
    if (!toInsert.length) {
      return NextResponse.json({
        error: `Keine gültigen Sensor-IDs gefunden. Geprüfte IDs: ${sensorIds.join(', ')}. Gültige IDs in Org: ${(validSensors ?? []).map(s => s.id).join(', ') || 'keine'}`,
      }, { status: 400 })
    }
    ...
    // Unerwartete Fehler abfangen damit nie eine TCP-Verbindung abrupt geschlossen wird
    console.error('[/api/sensors/ingest] Unhandled error:', err)
    return NextResponse.json(
...
// Health-Check — testet ob der Endpunkt erreichbar ist
export async function GET() {
  return NextResponse.json({ ok: true, endpoint: 'sensor-ingest' })
}
