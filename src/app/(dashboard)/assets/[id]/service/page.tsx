    ...
    <div style={{ fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ padding: '20px 20px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <h1 style={{ fontSize: 20, fontWeight: 700, color: '#000', margin: 0 }}>Serviceheft</h1>
        </div>
        <Link href={`/assets/${id}/service/neu`} style={{
          ...
          <span style={{ fontSize: 16 }}>+</span> Eintrag
        </Link>
      </div>

      {/* Statistik-Karten */}
      <div style={{ padding: '0 20px 20px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
        <StatCard label="Einträge" value={String(events?.length ?? 0)} icon={<ClipboardList size={14} />} />
        <StatCard label="Gesamtkosten" value={totalCost > 0 ? `${totalCost.toLocaleString('de-DE')} €` : '–'} icon={<Euro size={14} />} />
        <StatCard label="Intervalle" value={String(schedules?.length ?? 0)} icon={<RefreshCw size={14} />} />
      </div>
      ...
      <div style={{ padding: '20px 20px 0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <SectionTitle icon={<RefreshCw size={14} />} label="Wartungsintervalle" />
          <Link href={`/assets/${id}/service/intervall`} style={{
            ...
            <span style={{ fontSize: 14 }}>+</span> Intervall
          </Link>
        </div>
        {schedules && schedules.length > 0 ? (
          ...
          <div style={{ background: 'white', borderRadius: 12, padding: '20px 16px', border: '1px dashed #c8d4e8', textAlign: 'center' }}>
            <p style={{ fontSize: 13, color: '#96aed2', margin: '0 0 12px' }}>Noch kein Intervall angelegt</p>
            <Link href={`/assets/${id}/service/intervall`} style={{
              backgroundColor: '#003366', color: 'white', padding: '9px 20px', borderRadius: 50,
              textDecoration: 'none', fontSize: 13, fontWeight: 700,
            }}>+ Erstes Intervall anlegen</Link>
          </div>
        )}
      </div>
