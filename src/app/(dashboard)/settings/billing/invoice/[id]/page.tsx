      ...
      <div style={{ maxWidth: 740, margin: '0 auto', padding: '40px 40px 60px', background: 'white', color: '#000' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 40 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>Inomet GmbH</div>
          </div>
          <div style={{ textAlign: 'right', fontSize: 12, color: '#333', lineHeight: 1.7 }}>
            ...
            <div>USt-ID: {SELLER.vat_id}</div>
            <div>St-Nr.: {SELLER.tax_number}</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 40, gap: 40 }}>
          <div>
            <div style={{ fontSize: 10, color: '#888', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Rechnungsempfänger</div>
            <div style={{ fontSize: 13, lineHeight: 1.8 }}>
              ...
              <div>{invoice.billing_country ?? 'Deutschland'}</div>
              {invoice.billing_vat_id && <div>USt-ID: {invoice.billing_vat_id}</div>}
            </div>
          </div>
          <div style={{ textAlign: 'right', fontSize: 12, lineHeight: 2 }}>
            <div><span style={{ color: '#888' }}>Rechnungsnummer:</span> <strong>{invoice.invoice_number}</strong></div>
            <div><span style={{ color: '#888' }}>Rechnungsdatum:</span> {formatDate(issueDate)}</div>
            <div><span style={{ color: '#888' }}>Zahlungsziel:</span> {formatDate(dueDateObj)}</div>
            <div>
              <span style={{ color: '#888' }}>Status:</span>{' '}
              <span style={{
                fontWeight: 700,
                color: invoice.status === 'paid' ? '#166534' : invoice.status === 'cancelled' ? '#991b1b' : '#854d0e',
              }}>
                {invoice.status === 'paid' ? 'Bezahlt' : invoice.status === 'cancelled' ? 'Storniert' : 'Ausstehend'}
              </span>
            </div>
          </div>
        </div>

        <h2 style={{ fontSize: 20, fontWeight: 700, color: '#003366', margin: '0 0 24px' }}>Rechnung</h2>

        {/* Line items table */}
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 24, fontSize: 13 }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #003366' }}>
              <th style={{ textAlign: 'left', padding: '8px 0', color: '#003366', fontWeight: 700 }}>Beschreibung</th>
              <th style={{ textAlign: 'right', padding: '8px 0', color: '#003366', fontWeight: 700 }}>Betrag (netto)</th>
            </tr>
          </thead>
          <tbody>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 32 }}>
          <div style={{ minWidth: 280, fontSize: 13 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', color: '#555' }}>
              <span>Nettobetrag</span><span>{fmt(invoice.amount_net)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', color: '#555' }}>
              <span>zzgl. {Math.round((invoice.vat_rate ?? 0.19) * 100)} % MwSt.</span>
            ...
            <div style={{
              ...
              <span>Gesamtbetrag</span><span>{fmt(invoice.amount_gross)}</span>
            </div>
          </div>
        </div>
        ...
        {invoice.status !== 'paid' && (
          <div style={{
            ...
            <div style={{ fontWeight: 700, color: '#003366', marginBottom: 8 }}>Bankverbindung</div>
            <div style={{ lineHeight: 2, color: '#333' }}>
              <div><span style={{ color: '#888' }}>Empfänger:</span> {SELLER.name}</div>
              <div><span style={{ color: '#888' }}>Bank:</span> {SELLER.bank_name}</div>
              <div><span style={{ color: '#888' }}>IBAN:</span> <strong>{SELLER.iban}</strong></div>
              <div><span style={{ color: '#888' }}>BIC:</span> {SELLER.bic}</div>
              <div><span style={{ color: '#888' }}>Verwendungszweck:</span> <strong>{invoice.invoice_number}</strong></div>
            </div>
          </div>
        )}
        ...
        {invoice.status === 'paid' && invoice.activation_code && (
          <div style={{
            ...
            <div style={{ fontWeight: 700, color: '#166534', marginBottom: 6 }}>Aktivierungscode</div>
            ...
            <div style={{ fontSize: 11, color: '#666', marginTop: 6 }}>
              Geben Sie diesen Code unter Abonnement → Aktivierungscode ein, um Ihren Plan zu aktivieren.
            </div>
          </div>
        )}
        ...
        <div style={{ fontSize: 11, color: '#888', lineHeight: 1.7, borderTop: '1px solid #e5e7eb', paddingTop: 20 }}>
          <p style={{ margin: '0 0 6px' }}>
            Diese Rechnung wurde gemäß §14 UStG ausgestellt. Der Rechnungsaussteller ist im deutschen Handelsregister eingetragen.
          </p>
          <p style={{ margin: 0 }}>
            Bei Fragen wenden Sie sich bitte an: {SELLER.email}
          </p>
        </div>
