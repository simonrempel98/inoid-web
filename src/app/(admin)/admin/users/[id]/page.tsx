  ...
  return (
    <div style={{ maxWidth: 560 }}>
      <div style={{ marginBottom: 24 }}>
        <Link href="/admin/users" style={{ color: 'var(--adm-text3)', fontSize: 13, textDecoration: 'none' }}>
          ← Nutzer
        </Link>
        <h1 style={{ fontSize: 22, fontWeight: 900, color: 'var(--adm-text)', margin: '8px 0 4px' }}>
          {profile.full_name ?? profile.email}
        </h1>
        <p style={{ fontSize: 12, color: 'var(--adm-text3)', margin: 0 }}>{profile.email}</p>
      </div>

      <div style={{ background: 'var(--adm-surface)', borderRadius: 14, border: '1px solid var(--adm-border)', padding: '20px', marginBottom: 16 }}>
        <h2 style={{ fontSize: 13, fontWeight: 700, color: 'var(--adm-text3)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 14px' }}>Details</h2>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[
            ...
            <div key={String(label)}>
              <p style={{ margin: '0 0 2px', fontSize: 11, color: 'var(--adm-text3)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{label}</p>
              <p style={{ margin: 0, fontSize: 14, color: 'var(--adm-text)' }}>{value}</p>
            </div>
          ))}
        </div>
      </div>
